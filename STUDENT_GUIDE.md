...
這批資料裡我們偷偷藏了四個明顯壞人：有人在打 SQL Injection、有人在猜檔案路徑、有人在掃後門、有人在瘋狂爬資料——這四個都留下了明顯的痕跡，等一下就是要讓 Agent 自己把他們揪出來。（放心，公司名稱都是虛構的，IP 也是網路標準保留給教學文件用的假網段。）

![圖 2：資料裡藏的四個攻擊](images/fig2_attacks.svg)

*圖 2：合成資料刻意埋了四種容易辨識的攻擊，每個對應一家公司、一個來源 IP 與風險分數，等一下讓 Agent 自己找出來。*
...
- **SQL Injection**：攻擊者想把資料庫語法塞進參數裡。`union … select … from` 是想把另一張表（例如帳密表）的內容一起撈出來；`' or '1'='1` 是讓登入條件永遠成立；`--` 是把後面正常的 SQL 整段變成註解。規則找的就是這些語法碎片。
- **路徑遍歷**：`../` 的意思是「上一層目錄」，連打十幾個就能跳出網站根目錄，去讀 `/etc/passwd`（Linux 的帳號清單）這種系統檔。指紋就是一串 `../` 加上敏感檔名。
- **敏感路徑存取**：攻擊前的踩點。拿字典檔到處亂敲 `/.env`、`/.git`、`/phpmyadmin` 這些不該公開的路徑，敲到一個就撿到寶。
...
也誠實說出它的極限：這種做法叫「特徵比對」，只抓得到已知的攻擊手法，全新的手法或刻意變形的 payload 會漏接。所以真實世界的防守是分層的——特徵比對抓已知（便宜、準、可解釋），行為異常抓未知（上面的爆量偵測就是迷你版），最後由人或 AI 研判可疑線索。而第三層，正是等一下 LLM 要扮演的角色。
